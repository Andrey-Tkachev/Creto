  ...
  req.currentUser.save();
    UserModel.findById(req.session.user_id, function(err, user) {
      if (user) {
        ...
        res.render('../public/html/friends.html', 
                                        {'self': true,
                                            'person' :  {
...
function friends(req, res) {
  ...
  .populate({path: 'friends', select: 'thumb first_name last_name full_name'})
...
friend_request = { 
  request : function (req, res) {
    ...
    UserModel.findById(req.body.id,  function(err, user) {
      if ((user) && (user.friends_requests.indexOf(req.currentUser._id) == -1)) {
        UserModel.update({ _id: req.body.id }, {$pushAll: {friends_requests : [req.currentUser]}}, {upsert:true}, 
          function(e) { 
            if (e) {
              res.sendStatus(500);
            }
...
function delete_from_friend(req, res) {
}
...
module.exports.delete_from_friend = delete_from_friend;